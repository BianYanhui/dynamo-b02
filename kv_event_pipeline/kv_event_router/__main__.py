...
"""Selective KV-state gateway and Dynamo router service.

The current KV event design follows the paper's control-plane model:

    Instance reporter -> selective state gateway -> dispatcher hint
    Client -> Frontend -> KV Event Router -> native KvRouter -> workers

The native KvRouter remains the request/data-plane path.  The KV event gateway
...
Usage:
    PYTHONPATH=<repo>/kv_event_pipeline python -m kv_event_router \
...
from kv_event_router.policy import DispatchDecision, SketchDispatchPolicy, SketchPolicyConfig
from kv_event_router.selective_signaling import (
    KVStateUpdate,
    OwnerStateRegistry,
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="KV Event Router (Dynamo component)")
    ...
    ap.add_argument("--served-model-name", default=None,
                    help="model name this router registers (must differ from the "
                         "workers' own chat surface; e.g. qwen-kv)")
    ...
    ap.add_argument("--state-log-dir", default="/tmp/kv_event_state_logs")
    ...
    ap.add_argument("--zmq-relay-ports", default=None,
                    help="comma-separated vLLM raw ZMQ ports to relay through KV event")
    ...
    if args.served_model_name is None:
        args.served_model_name = f"{args.model_name}-kv"
...
def _extract_workflow_id(request: dict[str, Any]) -> tuple[Optional[str], str]:
    ...
    if isinstance(nv, dict):
        for key in ("kv_workflow_id", "session_id", "workflow_id"):
            v = nv.get(key)
            if isinstance(v, str) and v:
                return v, "nvext"
    ea = request.get("extra_args")
    if isinstance(ea, dict):
        for key in ("kv_workflow_id", "workflow_id"):
            v = ea.get(key)
            if isinstance(v, str) and v:
                return v, "extra_args"
    return None, "none"
...
class KVEventRouterHandler:
    ...
    async def initialize(self) -> None:
        ...
        if self._args.zmq_relay_ports:
            ...
            if len(worker_ids) != len(ports):
                raise RuntimeError(
                    f"KV event ZMQ relay expected {len(ports)} workers, found {worker_ids}"
                )
            if self._args.zmq_relay_shards == 1:
                ...
                logger.info("KV event ZMQ selective relay started (workers=%s, ports=%s)",
                            worker_ids, ports)
            else:
                self._start_sharded_relays(worker_ids, ports)
        logger.info("KV event sketch router initialized (endpoint=%s, block_size=%s)",
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("KV event sketch router shutdown complete")
    ...
    def _start_sharded_relays(self, worker_ids: list[int], ports: list[int]) -> None:
        ...
        package_root = os.path.join(repo_root, "kv_event_pipeline")
        ...
        for shard in range(shard_count):
            ...
            logger.info("KV event ZMQ relay shard started (pid=%s, workers=%s, ports=%s)",
        ...
        self._zmq_relay_ready_files.clear()
        self._zmq_relay_stats_files.clear()
        raise RuntimeError("timed out waiting for process-sharded KV event relays")
    ...
    async def generate(self, request: dict[str, Any]):
        if self._kv_router is None:
            raise RuntimeError("KVEventRouterHandler used before initialize()")

        if not self._keys_logged:
            self._keys_logged = True
            logger.info("kv_event.request_keys keys=%s", sorted(request.keys()))
        ...
        logger.info("kv_event.dispatch workflow=%s step=%d source=%s reason=%s pin=%s "
...
async def worker(runtime: DistributedRuntime) -> None:
    args = parse_args()
    logger.info("KV event sketch router starting (backend=%s, served_as=%s)",
                args.endpoint, args.served_model_name)

    handler = KVEventRouterHandler(runtime, args)
    await handler.initialize()

    generate_endpoint = runtime.endpoint(f"{args.namespace}.kv_event_router.generate")
    status_endpoint = runtime.endpoint(f"{args.namespace}.kv_event_router.status")
    signal_endpoint = runtime.endpoint(f"{args.namespace}.kv_event_router.kv_state")
    ...
    try:
        ...
        await register_model(**reg_kwargs)
    logger.info("KV event sketch router registered model '%s' (tokenizer from %s)",
    ...
    try:
        await asyncio.gather(
            generate_endpoint.serve_endpoint(
                ...
                metrics_labels=[("service", "kv_event_router")]),
            ...
            signal_endpoint.serve_endpoint(
                ...
                graceful_shutdown=True,
                metrics_labels=[("service", "kv_selective_state_gateway")]),
        )
    finally:
