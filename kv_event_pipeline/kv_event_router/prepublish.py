    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._measure_wire_bytes = (
            os.environ.get("DYN_KV_EVENT_PREPUBLISH_MEASURE_BYTES", "0") == "1"
        )
...
def install_kv_event_prepublish() -> bool:
    ...
    logger.info(
        "KV event worker-local pre-publish enabled (flush_ms=%s, max_events=%s, measure_wire_bytes=%s)",
        os.environ.get("DYN_KV_EVENT_PREPUBLISH_FLUSH_MS", "2.0"),
        os.environ.get("DYN_KV_EVENT_PREPUBLISH_MAX_EVENTS", "4096"),
        os.environ.get("DYN_KV_EVENT_PREPUBLISH_MEASURE_BYTES", "0"),
    )
    return True
