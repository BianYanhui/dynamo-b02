...
"""Raw vLLM ZMQ -> selective KV event gateway -> Dynamo KV event publisher.
...
authority and does not need a specialized decoder.  When several stored
...
from kv_event_router.selective_signaling import KVStateUpdate, SelectiveKVStateGateway
...
class RawZmqSelectiveRelay:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="kv-event-zmq-selective-relay",
            daemon=True,
        )
        self._thread.start()
