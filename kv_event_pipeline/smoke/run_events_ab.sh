...
#   cell kv_event : frontend -> KV event sketch router (affinity pin)
set -u
...
STATE_DIR=/tmp/kv_event_events_ab
...
stop_router() { pkill -f 'kv_event[_]router' 2>/dev/null; sleep 2; }
...
start_router() {
  ...
  setsid nohup env PYTHONPATH=$EVENT_PIPELINE python -m kv_event_router \
      ...
      --model-path $SNAPSHOT --served-model-name qwen-kv \
      --state-log-dir /tmp/kv_event_state_logs \
...
try: print(','.join(m.get('id','') for m in json.load(sys.stdin).get('data',[])))
    ...
    case "$MODELS" in *qwen-kv*) echo "  router serving qwen-kv"; return 0;; esac
    sleep 2
  done
  echo "FAIL: qwen-kv not served"; return 1
}
...
start_tap() { # $1 tag
  setsid nohup python $EVENT_PIPELINE/smoke/event_tap.py \
...
run_load() { # $1 tag $2 model $3 seed
  python $EVENT_PIPELINE/smoke/smoke_client.py --model $2 --n-workflows 8 --steps 8 \
...
echo "=== [cell 3/3] KV event sketch router (affinity pin) ==="
stop_router
start_frontend round-robin ""
start_router || exit 1
probe kv_event qwen-kv || exit 1
start_tap kv_event
run_load kv_event qwen-kv 13
stop_tap kv_event

echo "=== analysis ==="
python $EVENT_PIPELINE/smoke/analyze_events.py
echo "EVENT AB DONE"
