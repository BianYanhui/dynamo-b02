...
#   DYN_KV_EVENT_PREPUBLISH=1 -> filter KV events before the Worker ZMQ PUB
#   DYN_KV_EVENT_PREPUBLISH_BACKEND=rust|python|auto -> selector implementation
set -u
cd /home/byh/Dynamo
...
DYN_KV_EVENT_PREPUBLISH=${DYN_KV_EVENT_PREPUBLISH:-0}
DYN_KV_EVENT_PREPUBLISH_BACKEND=${DYN_KV_EVENT_PREPUBLISH_BACKEND:-auto}
WORKER_ENV=()
if [ "$DYN_KV_EVENT_PREPUBLISH" = "1" ]; then
  WORKER_ENV=(
    DYN_KV_EVENT_WORKER=1
    DYN_KV_EVENT_PREPUBLISH_BACKEND="$DYN_KV_EVENT_PREPUBLISH_BACKEND"
    PYTHONPATH="/home/byh/Dynamo/dynamo/kv_event_pipeline${PYTHONPATH:+:$PYTHONPATH}"
  )
fi
