#!/usr/bin/env python3
"""Stress native ZMQ and Worker-local KV event with real native envelope shapes.
...
into an artificial duplicate-only workload.  Native and KV event receive the same
    ...
    try:
        ...
        return {
            ...
            **receiver_stats,
            ...
            # KV event, output_events is already reduced by local selection, so
...
def main() -> None:
    ...
    ap.add_argument(
        "--modes",
        default="native,prepublish-python,prepublish-rust",
        help="native, prepublish-python, and/or prepublish-rust",
    )
    ...
    for rate in rates:
        for mode in [item.strip() for item in args.modes.split(",") if item.strip()]:
            if mode in {"native"}:
                backend = "python"
            elif mode in {"selective", "prepublish-python", "python"}:
                mode = "prepublish-python"
                backend = "python"
            elif mode in {"prepublish-rust", "rust"}:
                mode = "prepublish-rust"
                backend = "rust"
            else:
