...
STATE_DIR = os.environ.get("B02_STATE_LOG_DIR", "/tmp/b02_events_ab")
...
def main() -> int:
    ...
    b, k = stats["b02"].get("msgs_per_req"), stats["kv"].get("msgs_per_req")
    if b and k:
        print(f"   b02 vs native kv: {100 * (1 - b / k):+.1f}% reduction")
    if not all(stats[m].get("total_msgs") for m in modes):
        rc = 1
        print("   FAIL: missing tap data for some cells")
    if stats["rr"]["requests_ok"] == 0 or stats["b02"]["requests_ok"] == 0:
        rc = 1
